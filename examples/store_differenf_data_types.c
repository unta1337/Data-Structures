...
int main(void)
{
    ...
    unds_list_t* lst = unds_list_create(sizeof(void*));
    ...
    temp = unds_malloc(sizeof(void*));

    // 각 자료형에 맞는 자료형으로 캐스팅 및 역참조햐여 출력.
    unds_list_get(lst, temp, 0);
    printf("%d\n", **(int**)temp);

    unds_list_get(lst, temp, 1);
    printf("%f\n", **(float**)temp);

    unds_list_get(lst, temp, 2);
    printf("%lf\n", **(double**)temp);

    // 문자열 리터럴은 그 자체로 포인터이므로 다른 자료형들과는 다르게 역참조를 한 번만 하여 char*로 만들어준다.
    unds_list_get(lst, temp, 3);
    printf("%s\n", *(char**)temp);

    // 동적할당된 공간 반납.
    unds_free(temp);
    unds_list_delete(lst);
